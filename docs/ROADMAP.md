...
### Universal Links (https://...) — Tier 2.5

`lumen.linking.onIncoming` уже ловит deep links по custom scheme (`lumen://`). Universal Links — это когда `https://example.com/path` открывается **прямо в нашем app'е** вместо Safari. Apple-side требований больше:

- **Associated Domains entitlement** (`applinks:example.com`) — нужно подписать профилем разработчика с поддержкой capability.
- **`apple-app-site-association` JSON** на стороне домена по фиксированному пути.
- На JS-стороне ничего не меняется — те же `linking.onIncoming.subscribe(fn)`, потому что `.onOpenURL` ловит и UL.

Дорогая инфра-возня вокруг Apple, дешёвый код-импакт.

### APNS / remote push — Tier 2.5

`lumen.notifications.*` сейчас покрывает только **local** notifications (UNTimeIntervalNotificationTrigger). Push с сервера (APNS) требует:

- **`aps-environment` entitlement** в provisioning profile (development / production).
- **APNS-сертификат или auth-key** на сервере (`.p8` через JWT).
- Регистрация device token: `application.registerForRemoteNotifications` → `didRegisterForRemoteNotificationsWithDeviceToken`. У нас сейчас нет AppDelegate, надо ввести `@UIApplicationDelegateAdaptor` или захватить через scene.
- `lumen.notifications.deviceToken()` / `onDeviceTokenChange.subscribe(fn)` API на JS-стороне.
- Background fetch / silent push — дополнительные UIBackgroundModes.

Объём бекенд+ops, не код в app'е. Закроем когда будет конкретный кейс с серверной стороны.
